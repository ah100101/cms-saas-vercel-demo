...
export const CmsLink: FunctionComponent<CmsLinkProps> = ({
  href,
  ...props
}) => {
  ...
  // hack
  const linkHref =
    url?.host === "app-ocxcalha1wff87p003.cms.optimizely.com"
      ? url.pathname
      : urlToRelative(url);
  ...
  return url ? (
    <Link {...props} href={linkHref} title={linkTitle} target={linkTarget}>
      {linkText}
    </Link>
  ) : (
    <span {...props} title={linkTitle}>
      {linkText}
    </span>
  );
};
